...
type PagoConPeriodo = {
  ...
  comprobantes_pago: { id: string; pago_recibido: boolean }[]
}
...
export default async function ContratoPagosPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: contrato } = await db
    .from('contratos')
    .select('id, organizacion_id, monto_actual')
    .eq('id', contratoId)
    .single()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">{verificados} de {total} pagos verificados</p>
        </div>
        {/* Barra de progreso */}
