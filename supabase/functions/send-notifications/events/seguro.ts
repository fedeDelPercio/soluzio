// Handlers S2 (recordatorio a 15 días) y S4 (vencido).
...
import { describirPropiedad, ayer, haceDias } from '../lib/fechas.ts'
...
async function recolectarTargets(c: ContratoRow, emails: Map<string, string>) {
...
export async function handleSeguroRecordatorio(): Promise<ResultadoEvento> {
  ...
  for (const c of contratos) {
    ...
    for (const t of await recolectarTargets(c, emails)) {
      const { asunto, html } = buildSeguroRecordatorioEmail({
        esAdmin:          t.esAdmin,
      ...
      await reservarYEnviar({
        ...
        contexto_unico:    `contrato:${c.id}:seguro_recordatorio`,
        asunto, html,
        metadata:          { contrato_id: c.id, fecha_inicio: c.fecha_inicio },
      }, resultado)
    }
  }
  return resultado
...
export async function handleSeguroVencido(): Promise<ResultadoEvento> {
  ...
  const { data: contratosRaw, error } = await supabase
    ...
    .select(`
      ...
      coinquilino:perfiles!contratos_coinquilino_id_fkey ( id, nombre, apellido )
    `)
    .eq('vencimiento_seguro_incendio', ayer())
    .in('estado', ['activo', 'por_vencer'])
  ...
  for (const c of contratos) {
    ...
    const inqNombre = c.inquilino ? `${c.inquilino.nombre} ${c.inquilino.apellido}`.trim() : 'el inquilino'
    for (const t of await recolectarTargets(c, emails)) {
      const { asunto, html } = buildSeguroVencidoEmail({
        esAdmin:          t.esAdmin,
        inquilinoNombre:  t.esAdmin ? inqNombre : undefined,
        propiedad:        propiedadStr,
        contratoId:       c.id,
        fechaVencimiento: c.vencimiento_seguro_incendio,
      })
      await reservarYEnviar({
        evento,
        organizacion_id:   c.organizacion_id,
        destinatario_id:   t.id,
        destinatarioEmail: t.email,
        contexto_unico:    `contrato:${c.id}:seguro_vencido:${c.vencimiento_seguro_incendio}`,
        asunto, html,
        metadata:          { contrato_id: c.id, fecha_vencimiento: c.vencimiento_seguro_incendio },
      }, resultado)
    }
  }
  return resultado
...
export const _eventoSeguroRecordatorio: EventoCritico = 'seguro_incendio_recordatorio'
export const _eventoSeguroVencido:      EventoCritico = 'seguro_incendio_vencido'
