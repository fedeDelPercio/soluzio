...
interface ContratoRow {
  id: string; organizacion_id: string; fecha_fin: string; estado: string;
...
export async function handleContratoPorVencer(): Promise<ResultadoEvento> {
  ...
  for (const [diasOffset, orgIds] of porOffset) {
    ...
    const ids: string[] = []
    for (const c of contratos) { if (c.inquilino?.id) ids.push(c.inquilino.id); if (c.coinquilino?.id) ids.push(c.coinquilino.id); if (c.propiedades?.propietario_id) ids.push(c.propiedades.propietario_id) }
    ...
    if (propIds.length > 0) { const { data: props } = await supabase.from('perfiles').select('id, nombre').in('id', propIds); for (const p of (props ?? []) as { id: string; nombre: string }[]) propMap.set(p.id, { nombre: p.nombre }) }
